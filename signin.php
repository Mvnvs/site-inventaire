...
<html lang="fr">
    <head>
        ...
        <title>Sign in</title>
        <link rel="stylesheet" type="text/css" href="logsin.css">
    ...
    <header class="relative bg-gradient-to-r from-cyan-600 to-cyan-100 w-screen">
        <nav class="mx-auto max-w-7xl px-4 h-sm:px-6 lg:px-8" aria-label="Top">
          <div class="flex w-full items-center justify-between border-b border-grey-500 py-6 lg:border-none">
            <div class="flex items-center">
              ...
              <div class="ml-10 hidden space-x-8 lg:block">
                <a href="gallery.php" class="text-base font-medium text-white hover:text-gray-500">Offers</a>
          ...
          <div class="flex flex-wrap justify-center space-x-6 py-4 lg:hidden">
            <a href="#" class="text-base font-medium text-white hover:text-gray-50">Solutions</a>
            <a href="#" class="text-base font-medium text-white hover:text-gray-50">Pricing</a>
            <a href="#" class="text-base font-medium text-white hover:text-gray-50">Docs</a>
            <a href="#" class="text-base font-medium text-white hover:text-gray-50">Company</a>
          </div>
        </nav>
    </header>
    ...
    <body >
        <div id="body">
            ...
            <form method="post">
                ...
                <div id="condition">
                    <p>At least 8 characters & 1 number</p> 
                </div>
                <input type="submit" name="formsend" id="formsend" value="Sign in">
                 ...
                 if(isset($_POST['formsend'])){
                     extract($_POST);
                     if(!empty($password) && !empty($cpassword) && !empty($cemail) && !empty($firstname) && !empty($lastname)){
                        $password = trim($_POST['password']);
                        if(strlen($password) < 8)
                            echo 'Password must be at least 8 characters';
                        else{

                            if( strpos(trim($_POST['password']), 1) || strpos(trim($_POST['password']), 0)|| strpos(trim($_POST['password']), 2) || strpos(trim($_POST['password']), 3)|| strpos(trim($_POST['password']), 4)|| strpos(trim($_POST['password']), 5)|| strpos(trim($_POST['password']), 6)|| strpos(trim($_POST['password']), 7)|| strpos(trim($_POST['password']), 8)|| strpos(trim($_POST['password']), 9)){
                                if ($password == $cpassword){
                                     ...
                                     $c->execute(['email' =>$cemail]);
                                     $result = $c->rowCount();

                                     if($result == 0){
                                         $mysql =$db->prepare("INSERT INTO utilisateur(name,surname,email,password) VALUES(:name,:surname,:email,:password)");
                                         $mysql ->execute([
                                             'name'=>$firstname,
                                             'surname'=>$lastname,
                                             'email'=>$cemail,
                                     ...
                                     echo "Please put the same password";
                                 }
                            }
                            else{
                                 echo "Password must contain at least a number";
                            } 
                        }
                     }         
                 }
